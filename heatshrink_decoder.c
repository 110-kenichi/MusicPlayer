...
/* Forward references. */
static inline uint16_t get_bits(heatshrink_decoder *hsd, uint8_t count);
static inline void push_byte(heatshrink_decoder *hsd, output_info *oi, uint8_t byte);
...
void heatshrink_decoder_reset(heatshrink_decoder *hsd) {
    ...
    //HACK: memset(hsd->buffers, 0, buf_sz + input_sz);
...
static inline HSD_state st_tag_bit(heatshrink_decoder *hsd) {
...
static inline HSD_state st_yield_literal(heatshrink_decoder *hsd,
        output_info *oi) {
...
static inline HSD_state st_backref_index_msb(heatshrink_decoder *hsd) {
...
static inline HSD_state st_backref_index_lsb(heatshrink_decoder *hsd) {
    uint8_t bit_ct = BACKREF_INDEX_BITS(hsd);
...
static inline HSD_state st_backref_count_msb(heatshrink_decoder *hsd) {
    uint8_t br_bit_ct = BACKREF_COUNT_BITS(hsd);
    ASSERT(br_bit_ct > 8);
...
static inline HSD_state st_backref_count_lsb(heatshrink_decoder *hsd) {
    uint8_t br_bit_ct = BACKREF_COUNT_BITS(hsd);
...
static inline HSD_state st_yield_backref(heatshrink_decoder *hsd,
        output_info *oi) {
